...
export default function BlogPostsList({ initialPosts }: BlogListProps) { // Accept initialPosts prop
  ...
  const handleEditPost = (slug: string) => {
    ...
    if (postToEdit) {
      ...
      setEditingPostData({
        ...
        tags: Array.isArray((postToEdit.metadata as any).tags) ? (postToEdit.metadata as any).tags : [],
  ...
  const handleSavePost = async (data: PostData) => { // Make the function async
    try {
    ...
    <>
      ...
      {posts.length === 0 ? (
        ...
        <ul className="h-[90%] overflow-y-auto">
          {posts.map((post) => (
            <li key={post.slug} className="border-b border-gray-200 py-2 flex justify-between items-center pie-25">
              <div>
                <h3 className="text-xl font-medium">{post.metadata.title}</h3>
                <p className="text-gray-600 text-sm">
                  Category: {post.metadata.category} | Published: {new Date(post.metadata.publishedAt).toLocaleDateString()}
                </p>
              </div>
              <div className="flex">
                <button
