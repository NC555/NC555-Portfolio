...
function isResumeSectionWithItems(
  section: any
): section is ResumeSectionWithItems {
  return (
    section &&
...
const ResumePagePreview: React.FC<ResumePagePreviewProps> = ({
  resumeData,
}) => {
  if (!resumeData) {
    return (
      <div className="text-center text-gray-500">Loading preview data...</div>
    );
  }
  ...
  return (
    <article>
      ...
      {resumeData.downloadSloganCV && (
        <DownloadCv downloadSloganCV={resumeData.downloadSloganCV} />
      )}
      <div className="space-y-8">
        ...
        {sectionsOrder.every((key) => {
          ...
          return (
            !isResumeSectionWithItems(section) || section.items.length === 0
          );
        }) && (
          <div className="text-center text-gray-500">
            No resume data available to display. Add items to the sections in
            the editor.
          </div>
        )}
      </div>
